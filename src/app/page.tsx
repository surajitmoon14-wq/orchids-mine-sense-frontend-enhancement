...
function UnderwaterBubble({ delay, left, size }: { delay: number; left: string; size: number }) {
  return (
    <motion.div
      className="absolute pointer-events-none rounded-full bg-gradient-to-br from-[#4fc3f7]/10 to-transparent border border-[#4fc3f7]/20"
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 h-32 overflow-hidden pointer-events-none">
      <motion.div
        ...
        style={{
          background: "linear-gradient(to top, rgba(79, 195, 247, 0.05), transparent)",
        }}
        animate={{ y: [0, -10, 0] }}
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {[0, 1, 2, 3].map((i) => (
        <motion.div
          key={i}
          className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#ff6b35]/10"
    ...
    <div className="min-h-screen bg-[#0a0a0f] overflow-hidden relative">
      ...
      {mounted && (
        <>
          ...
          {bubbles.map((b, i) => (
            <UnderwaterBubble key={`bubble-${i}`} {...b} />
          ))}
        </>
      )}
